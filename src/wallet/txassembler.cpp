    ...
    const bool sign)
{
    ...
    while (true) {
        ...
        // the recipients are funded through the pegout kernel instead of traditional LNGC outputs.
        ...
        // Calculate transaction size and the total necessary fee amount (includes LNGC and MWEB fees)
    ...
    if (new_tx.mweb_type != MWEB::TxType::LNGC_TO_LNGC) {
...
bool TxAssembler::AttemptCoinSelection(InProcessTx& new_tx, const CAmount& nTargetValue) const
{
    ...
    if (new_tx.recipients.front().IsMWEB()) {
        ...
        if (SelectCoins(new_tx, nTargetValue, params_pegin)) {
            return std::any_of(new_tx.selected_coins.cbegin(), new_tx.selected_coins.cend(), is_lngc);
        }
    } else {
        // First try to construct a LNGC-to-LNGC transaction
        CoinSelectionParams mweb_to_mweb = new_tx.coin_selection_params;
        mweb_to_mweb.input_preference = InputPreference::LNGC_ONLY;
        ...
        // If LNGC-to-LNGC fails, create a peg-out transaction
