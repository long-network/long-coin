Longcoin Core version 0.18.1 is now available from:

  <https://download.longcoin.org/longcoin-0.18.1/>
...
`/Applications/Longcoin-Qt` (on Mac) or `longcoind`/`longcoin-qt` (on
Linux).
...
Longcoin Core is supported and extensively tested on operating systems
using the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not
recommended to use Longcoin Core on unsupported systems.
...
10.10. Additionally, Longcoin Core does not yet change appearance when
...
- The `rpcallowip` option can no longer be used to automatically listen
  ...
  [...] -p 127.0.0.1:21251:21251` (this is an extra `:21251` over the
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](mailto:contact@longcoin.org) the Longcoin Core
  project to let us know about their use-case, as this feature could be
  deprecated in the future).

Wallet changes
--------------
When creating a transaction with a fee above `-maxtxfee` (default 0.1 LNGC),
the RPC commands `walletcreatefundedpsbt` and  `fundrawtransaction` will now fail
instead of rounding down the fee. Beware that the `feeRate` argument is specified
in LNGC per kilobyte, not litoshi per byte.
...
- A new [document](/blob/master/doc/longcoin-conf.md)
  about the `longcoin.conf` file describes how to use it to configure
  Longcoin Core.

- A new document introduces Longcoin Core's BIP174 [Partially-Signed
  Longcoin Transactions
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Longcoin-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Longcoin Core to libssl vulnerabilities in the past, builders who don't
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Longcoin Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Longcoin Core, subsequently importing
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart longcoind with the `-deprecatedrpc=generate`
...
- A new `longcoin-wallet` tool is now distributed alongside Longcoin
...
This section describes planned changes to Longcoin Core that may affect
other Longcoin software and services.

- Since version 0.16.0, Longcoin Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Longcoin Core 0.20 (expected about
  a year after 0.18), Longcoin Core will default to native segwit
  ...
  to bech32 addresses, and if the Longcoin Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Longcoin Core 0.20.
  ...
  set the `addresstype=bech32` configuration option in any Longcoin Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Longcoin Core
...
- A sub-project of Longcoin Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Longcoin Core.  See their [project
...
- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Longcoin Core's own implementation, although entropy
  gathered by Longcoin Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Longcoin Core a
...
- On macOS, Longcoin Core now opts out of application CPU throttling
...
- #21251 Let wallet `importmulti` RPC accept labels for standard scriptPubKeys (ryanofsky)
