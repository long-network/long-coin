...
The Longcoin Core daemon can be configured to act as a trusted "border
router", implementing the longcoin wire protocol and relay, making
...
The ZeroMQ feature in Longcoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building longcoind:
...
For instance:
    ...
    $ longcoind -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/longcoind.tx.raw \
...
These options can also be provided in longcoin.conf.
...
From the perspective of longcoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into longcoind directly. Furthermore, no information is
...
using. Longcoind appends an up-counting sequence number to each
